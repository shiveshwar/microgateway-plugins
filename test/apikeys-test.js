...
describe('apikey plugin', function () {
    ...
    it('checkIfAuthorized for  /a@@@@', function (done) {
        var contains;
        contains = apikey.checkIfAuthorized(withSpecialSymbol, `${proxy.base_path}/a`, proxy, token);  
        assert(!contains)
        contains = apikey.checkIfAuthorized(withSpecialSymbol, `${proxy.base_path}/a@@@@`, proxy, token);  
        assert(contains)
        contains = apikey.checkIfAuthorized(withSpecialSymbol, `${proxy.base_path}/asdas/b/c`, proxy, token);  
        assert(!contains)
        contains = apikey.checkIfAuthorized(withSpecialSymbol, `${proxy.base_path}/b/c`, proxy, token);  
        assert(!contains)
        contains = apikey.checkIfAuthorized(withSpecialSymbol, `${proxy.base_path}/a@@@`, proxy, token);  
        assert(!contains)
        contains = apikey.checkIfAuthorized(withSpecialSymbol, `${proxy.base_path}/@@@@`, proxy, token);  
        assert(!contains)
        done()
      })
      ...
      it('checkIfAuthorized for  /@@@@**', function (done) {
        var contains;
        contains = apikey.checkIfAuthorized(withStarAndSpecialSymbol, `${proxy.base_path}/a`, proxy, token);  
        assert(!contains)
        contains = apikey.checkIfAuthorized(withStarAndSpecialSymbol, `${proxy.base_path}/a@@@@`, proxy, token);  
        assert(!contains)
        contains = apikey.checkIfAuthorized(withStarAndSpecialSymbol, `${proxy.base_path}/@@@@`, proxy, token);  
        assert(contains)
        contains = apikey.checkIfAuthorized(withStarAndSpecialSymbol, `${proxy.base_path}/@@@@/dsfsd/sdfsdf`, proxy, token);  
        assert(contains)
        done()
      })
